...
const SettingsPage = () => {
  ...
  const fetchUserProfile = async () => {
    try {
      ...
      const profileRes = await axios.get(`https://server-io-psi.vercel.app/api/v1/profile/${userId}`);
      const mockRes = await axios.get(`https://server-io-psi.vercel.app/api/v1/user/${userId}`);
  ...
  const handleRequestEdit = async () => {
    try {
      ...
      const res = await axios.post("https://server-io-psi.vercel.app/api/v1/user/request-profile-edit", {
        userId,
        note: requestNote,
      });
