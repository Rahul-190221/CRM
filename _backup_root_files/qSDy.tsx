...
const BookingId = ({ params }: { params: { bookingId: string } }) => {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      ...
      try {
        const res = await axios.get(`https://server-io-psi.vercel.app/api/v1/user/${id}`);
        const data = res.data;
        ...
        const bookingsRes = await axios.get(
          `https://server-io-psi.vercel.app/api/v1/user/bookings/${id}`
        );
    ...
    const fetchScheduleData = async (selectedDate: Date) => {
      try {
        ...
        const response = await axios.get(
          `https://server-io-psi.vercel.app/api/v1/schedule/${formattedDate}/${params.bookingId}`
        );
  ...
  const handleProceed = async () => {
    ...
    if (isHome) {
      ...
      try {
        await axios.post(`https://server-io-psi.vercel.app/api/v1/user/book-slot`, {
          ...bookingPayload,
        ...
        if (oldBookingId) {
          await axios.delete(
            `https://server-io-psi.vercel.app/api/v1/bookings/${oldBookingId}`
          );
    ...
    try {
      ...
      if (oldBookingId) {
        await axios.delete(`https://server-io-psi.vercel.app/api/v1/bookings/${oldBookingId}`);
        toast.success("Rescheduled successfully! 🎯");
      } else {
