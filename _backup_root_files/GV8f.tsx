...
const Table = ({ userId }: { userId: string }) => {
  ...
  useEffect(() => {
    const fetchBookings = async () => {
      try {
        const response = await axios.get(`https://server-io-psi.vercel.app/api/v1/user/bookings/${userId}`);
  ...
  const handleViewResult = async (bookingId: string, bookingName: string) => {
    try {
      const response = await axios.get(`https://server-io-psi.vercel.app/api/v1/user/bookings/result/${bookingId}`);
